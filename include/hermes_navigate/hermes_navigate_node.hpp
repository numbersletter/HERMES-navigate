 ...
 *
 * Lifecycle behaviour:
 *   on_configure — Records start pose; loads BT plugins and builds the
 *                  HermesExplorationBT tree. Robot does not move yet.
 *   on_activate  — Starts the BT tick timer. Robot begins exploration.
 *   on_deactivate— Sets `return_to_start = true` on the BT blackboard so the
 *                  tree returns the robot to the start pose before halting.
 *   on_cleanup   — Destroys the tree and releases resources.
